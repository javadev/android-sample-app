...
public abstract class Tetrino {
    ...
    protected void resetGhost(int size) {
        for (int col = 0; col < size; col++) {
            for (int row = 0; row < size; row++) gMap[col][row] = TileView.BLOCK_EMPTY; // ghost Map
        }
    }
    ...
    public boolean rotateTetrino(TetrinoMap map) {
        ...
        if (!isColusionX(this.pos.x, temp, map)
                && !isColusionY(this.pos.y, this.pos.x, temp, map, false)) {
            sMap = temp;
            resetGhost(SIZE);
            copyTetrinoMap(temp, gMap, SIZE);
            setGhostY();
            return true;
        } else if (!isColusionX(this.pos.x - 1, temp, map)
                && !isColusionY(this.pos.y, this.pos.x - 1, temp, map, false)) {
            this.pos.x -= 1;
            this.ghostPos.x -= 1;
            sMap = temp;
            resetGhost(SIZE);
            ...
            for (int col = 0; col < this.getSize(); col++) {
                for (int row = 0; row < this.getSize(); row++) {
                    if (sMap[col][row] != TileView.BLOCK_EMPTY) {
                        if (x + col >= TetrinoMap.MAP_X_SIZE
                                || x + col < 0
    ...
    protected boolean isColusionY(
            int newY, int newX, int[][] tMap, TetrinoMap map, boolean isGhost) {
                        ...
                        if (isGhost) { // TODO need to think about if condition
                            if ((newX + col) >= 0 && (newX + col) < TetrinoMap.MAP_X_SIZE) {
                                if (newY + row >= TetrinoMap.MAP_Y_SIZE
                                        || map.getMapValue(newX + col, newY + row)
                ...
                for (int row = 0; row < this.getSize(); row++) {
                    if (tMap[col][row] != TileView.BLOCK_EMPTY) {
                        if (newX + col >= TetrinoMap.MAP_X_SIZE
                                || newX + col < 0
                                || map.getMapValue(newX + col, this.pos.y + row)
                                        != TileView.BLOCK_EMPTY) return true;
                    }
                }
            }
    ...
    public void drop(TetrinoMap map) {
        if (ghostEnabled) this.pos.y = this.ghostPos.y;
        else
            for (int y = 0;
                    y < TetrinoMap.MAP_Y_SIZE && !isColusionY(y, this.pos.x, sMap, map, false);
                    y++) this.pos.y = y;
    }
    ...
    protected void copyTetrinoMap(int[][] srcMap, int[][] destMap, int size) {
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                if (srcMap[x][y] != TileView.BLOCK_EMPTY) destMap[x][y] = TileView.BLOCK_GHOST;
            }
        }
    }
    ...
    public boolean onGhost() {
        if (pos.y == ghostPos.y) return true;
        return false;
    }
}
